...
# Assistente all'installazione Gentoo Linux, allineato alle fasi del manuale ufficiale
# (Handbook:AMD64 — Installazione: rete, dischi, stage3, base, kernel, sistema, strumenti, bootloader, finalizzazione).
#
# Uso (da live environment minimal, come root):
#   bash gentoo-install-assistant.sh
#   bash gentoo-install-assistant.sh --config /path/to/gentoo-install.conf
#   bash gentoo-install-assistant.sh --dry-run
#   bash gentoo-install-assistant.sh --from-step stage3
#
...
# e riduce errori di copia-incolla, lasciando scelte importanti all'utente.
#
# Esempio gentoo-install.conf (bash, caricabile con --config):
#   GENTOO_ROOT=/mnt/gentoo
#   GENTOO_ARCH=amd64
#   GENTOO_INIT=openrc
#   GENTOO_STAGE_FLAVOR=          # oppure: desktop
#   GENTOO_NOMULTILIB=0
#   GENTOO_MIRROR=https://distfiles.gentoo.org
#   # GENTOO_STAGE_INDEX_URL=... # se il mirror usa nomi diversi da latest-stage3-*.txt
#   COMMON_FLAGS='-O2 -pipe'
#   MAKEOPTS='-j8'
#   TIMEZONE=Europe/Rome
#   LOCALES='it_IT.UTF-8 UTF-8|en_US.UTF-8 UTF-8'
#   HOSTNAME=gentoo-box
#   KERNEL_MODE=genkernel         # menuconfig | genkernel | binpkg | skip
...
# ---------------------------------------------------------------------------
# Architettura mirror (stage3 / snapshot): amd64, arm64, ...
GENTOO_ARCH="${GENTOO_ARCH:-amd64}"

# Init: openrc | systemd
GENTOO_INIT="${GENTOO_INIT:-openrc}"

# Profilo stage: es. desktop, musl, hardened (lasciare vuoto per stage "standard" multilib)
GENTOO_STAGE_FLAVOR="${GENTOO_STAGE_FLAVOR:-}"

# Se 1, cerca tarball con "nomultilib" nel nome (solo amd64; leggere il manuale prima)
...
# MAKEOPTS e COMMON_FLAGS (esempi sensati; adattare alla CPU)
...
# Righe locale.gen: una riga logica con "locale charset", più voci separate da |
LOCALES="${LOCALES:-en_US.UTF-8 UTF-8}"

# Hostname
HOSTNAME="${HOSTNAME:-gentoo}"

# Kernel: menuconfig | genkernel | binpkg | skip
KERNEL_MODE="${KERNEL_MODE:-menuconfig}"
...
# Pacchetti extra dopo il kernel (spazio separato)
EXTRA_PACKAGES="${EXTRA_PACKAGES:-app-admin/syslog-ng app-editors/vim}"
...
Variabili principali (export o file --config):
  GENTOO_ROOT GENTOO_ARCH GENTOO_INIT GENTOO_MIRROR GENTOO_STAGE_FLAVOR
  GENTOO_NOMULTILIB GENTOO_STAGE_INDEX_URL MAKEOPTS COMMON_FLAGS
  TIMEZONE LOCALES HOSTNAME KERNEL_MODE BOOTLOADER ESP_DEVICE GRUB_BIOS_DISK
  PORTAGE_PROFILE EXTRA_PACKAGES NONINTERACTIVE DO_PORTAGE_SYNC
...
Questo script guida le fasi dell'installazione descritte nel Gentoo Handbook (arch: ${GENTOO_ARCH}).
- Partizionamento, crittografia e LVM: responsabilità dell'utente (documentazione ufficiale).
...
step_stage3() {
  log "=== Stage3 (Handbook: the stage file) ==="
  prompt_val "$GENTOO_ARCH" GENTOO_ARCH "Architettura (releases/...)"
  prompt_val "$GENTOO_INIT" GENTOO_INIT "Init system (openrc|systemd)"
...
write_make_conf() {
  ...
  cat >"$mc" <<EOF
# Generato da gentoo-install-assistant.sh — rivedere CFLAGS, USE e mirror (manuale Portage)
...
step_portage() {
  ...
  prompt_val "$MAKEOPTS" MAKEOPTS "MAKEOPTS"
  prompt_val "$COMMON_FLAGS" COMMON_FLAGS "COMMON_FLAGS (es. -march=native -O2 -pipe)"
  write_make_conf
}
...
step_timezone() {
  log "=== Timezone ==="
  prompt_val "$TIMEZONE" TIMEZONE "Timezone (es. Europe/Rome)"
  if [[ "$DRY_RUN" -eq 0 ]]; then
...
step_locale() {
  log "=== Locale ==="
  prompt_val "$LOCALES" LOCALES "locale.gen: voci separate da | (es. it_IT.UTF-8 UTF-8|en_US.UTF-8 UTF-8)"
  if [[ "$DRY_RUN" -eq 0 ]]; then
...
step_kernel() {
  log "=== Kernel (Handbook: configuring the kernel) ==="
  prompt_val "$KERNEL_MODE" KERNEL_MODE "Modalità kernel: menuconfig|genkernel|binpkg|skip"
  case "$KERNEL_MODE" in
    skip)
      warn "Kernel saltato: ricordarsi di installarlo prima del riavvio."
      ;;
    menuconfig)
      chroot_exec "emerge -av sys-kernel/gentoo-sources" || true
      warn "Esecuzione menuconfig e build kernel nel chroot (lungo)..."
      if [[ "$DRY_RUN" -eq 0 ]]; then
        chroot "$GENTOO_ROOT" /bin/bash -c 'set -e; cd /usr/src/linux; make menuconfig; make -j"$(nproc)"; make modules_install; make install' || \
          warn "Compilazione kernel fallita o incompleta — completare a mano (Handbook kernel)."
      fi
      ;;
    genkernel)
      chroot_exec "emerge -av sys-kernel/genkernel sys-kernel/gentoo-sources"
      chroot_exec "genkernel all"
      ;;
    binpkg)
      chroot_exec "emerge -av sys-kernel/gentoo-kernel-bin" || warn "Kernel binario: verificare pacchetti disponibili sul profilo corrente."
      ;;
    *)
      err "KERNEL_MODE sconosciuto: $KERNEL_MODE"
      ;;
  esac
}
