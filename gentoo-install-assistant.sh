...
# -march=native è appropriato; con GCC recente si può sostituire con -march=raptorlake
# in make.conf se si preferisce un -march esplicito (vedi man gcc).
#
...
step_kernel() {
  ...
  if [[ "$DRY_RUN" -eq 0 ]]; then
    ...
    # installkernel-gentoo: USE dracut per generare initramfs con dracut all'install del kernel
    cat >"${GENTOO_ROOT}/etc/portage/package.use/50-installkernel-dracut" <<'PKUSE'
