...
# Dischi: default DISK_AUTO=1 su /dev/sda (intero disco), override in config.
#
...
#   # Dischi: default gia DISK_AUTO=1 e DISK_TARGET=/dev/sda (nulla da mettere se il tuo disco e sda)
#   # DISK_TARGET=/dev/nvme0n1   # se non e sda
#   # DISK_AUTO=0               # per partizionare a mano
#   DISK_ESP_MIB=512
#   DISK_SWAP_MIB=8192
...
# Partizionamento automatico (Handbook: GPT, EFI vfat, swap, root XFS). Default: attivo su /dev/sda (disco intero).
# Override: DISK_TARGET=/dev/nvme0n1 o DISK_AUTO=0 per partizione manuale.
DISK_AUTO="${DISK_AUTO:-1}"
DISK_TARGET="${DISK_TARGET:-/dev/sda}"
...
# Se il disco risulta removibile (es. chiavetta USB), lo script si ferma salvo DISK_ALLOW_REMOVABLE=YES
DISK_ALLOW_REMOVABLE="${DISK_ALLOW_REMOVABLE:-}"
...
Variabili principali (export o file --config):
  ...
  DISK_AUTO DISK_TARGET DISK_ESP_MIB DISK_SWAP_MIB DISK_WIPE_CONFIRM DISK_ALLOW_REMOVABLE
  PARTS_STATE DISK_ROOT_PART DISK_ESP_PART DISK_SWAP_PART
...
Default: DISK_AUTO=1, DISK_TARGET=/dev/sda (disco intero). GPT, EFI vfat, swap, root XFS; mount/swapon
su /mnt/gentoo; fstab UUID. NONINTERACTIVE: DISK_WIPE_CONFIRM=YES. Disco removibile: serve
DISK_ALLOW_REMOVABLE=YES o scegliere un altro DISK_TARGET.
Con DISK_AUTO=0: DISK_ROOT_PART (e opzionalmente DISK_ESP_PART, DISK_SWAP_PART) nel .conf o prompt allo step disks.
EOF
}
...
# lsblk RM=1: spesso chiavetta USB; evita wipe accidentale della live.
disk_target_is_removable() {
  local disk=$1 rm
  rm=$(lsblk -d -n -o RM "$disk" 2>/dev/null | tr -d '[:space:]')
  [[ "$rm" == "1" ]]
}
...
- Dischi: di default DISK_AUTO=1 sul disco intero /dev/sda (cambia con DISK_TARGET=). Crea GPT, EFI, swap, root XFS; poi mount e swapon su /mnt/gentoo. DISK_AUTO=0 per flusso manuale.
...
step_disks() {
  ...
  if [[ "${DISK_AUTO:-0}" == "1" ]]; then
    ...
    disk_is_whole_block "$DISK_TARGET" || err "DISK_TARGET deve essere un disco intero (es. /dev/sda o /dev/nvme0n1), non una partizione tipo /dev/sda1."
    disk_target_is_removable "$DISK_TARGET" && [[ "${DISK_ALLOW_REMOVABLE:-}" != "YES" ]] && \
      err "DISK_TARGET=${DISK_TARGET} risulta REMOVIBILE (probabile chiavetta USB). Per evitare di cancellare la live: usa il disco interno (es. nvme) o imposta DISK_ALLOW_REMOVABLE=YES se e voluto."
    ...
    if [[ "$NONINTERACTIVE" -eq 1 ]]; then
      ...
      echo "Disco obiettivo (default /dev/sda, override con DISK_TARGET): ${DISK_TARGET}"
      prompt_yn y "Confermi la DISTRUZIONE di tutti i dati su ${DISK_TARGET}?" || err "Operazione annullata."
    fi
  ...
  echo "Default script: DISK_AUTO=1 e DISK_TARGET=/dev/sda. Per disattivare: DISK_AUTO=0. Altro disco: DISK_TARGET=/dev/nvme0n1. NONINTERACTIVE: DISK_WIPE_CONFIRM=YES."
  if ! prompt_yn y "Ho gia creato e formattato le partizioni (root XFS) necessarie"; then
    err "Completare il partizionamento o usare DISK_AUTO=0 per inserire i device, poi rilanciare con --from-step mount"
  fi
